...
import {
  useBundleMedia,
  usePresignBundleMedia,
  useCompleteBundleMedia,
  useReorderBundleMedia,
  useSetPrimaryBundleMedia,
  useDeleteBundleMedia,
} from "@/hooks/useBundleMedia";
...
interface BundleMediaManagerProps {
  bundleId: string;
}
...
export function BundleMediaManager({ bundleId }: BundleMediaManagerProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [uploads, setUploads] = useState<UploadProgress[]>([]);
  const [globalError, setGlobalError] = useState<string | null>(null);

  const { data: media = [], isLoading } = useBundleMedia(bundleId);
  const presign = usePresignBundleMedia(bundleId);
  const complete = useCompleteBundleMedia(bundleId);
  const reorder = useReorderBundleMedia(bundleId);
  const setPrimary = useSetPrimaryBundleMedia(bundleId);
  const deleteMedia = useDeleteBundleMedia(bundleId);
    ...
    <div className="space-y-4">
      ...
      {hasMaxImages && (
        <p className="text-xs text-amber-600">
          Maximum of {MAX_IMAGES} images reached for this bundle.
        </p>
      )}
      {hasVideo && !hasMaxImages && (
        <p className="text-xs text-amber-600">
          A video is already attached. Only one video is allowed per bundle.
        </p>
      )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Upload up to {MAX_IMAGES} images and 1 video
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
